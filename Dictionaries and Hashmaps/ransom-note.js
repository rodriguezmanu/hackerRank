...
function checkMagazine(magazine, note) {
  ...
  for (let index = 0; index < note.length; index++) {
    const wordNote = note[index];
    if (magazine.includes(wordNote)) {
      const i = magazine.findIndex(item => item === wordNote);

      magazine.splice(i, 1);
      matches.push(wordNote);
    }
  }
  return matches.length === note.length ? console.log('Yes') : console.log('No');
}
