...
function isValid(s) {
  ...
  const aa = Object.values(obj).sort();
  const len = aa.length;
  const firstItem = aa[0];
  const lastItem = aa[len - 1];

  return firstItem === lastItem
    ? 'YES'
    : (firstItem === aa[len - 2] && lastItem - 1 === firstItem) ||
      (aa[1] === lastItem && firstItem === 1)
    ? 'YES'
    : 'NO';
}

console.log(isValid('aabbccddeefghi')); // NO 14/8
